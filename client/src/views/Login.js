...
const LoginSignup = () => {
    ...
    useEffect(() => {
        ...
        if (storedUsers.length === 0) {
            const admin = { username: 'admin', email: 'mazen.walid251@gmail.com', password: 'admin251' };
    ...
    const validateForm = () => {
        ...
        if (!newUser.username || newUser.username.length < 5 || newUser.username.length > 15) {
            swal.fire("Validation error", "Username must be between 5 and 15 characters.", "error");
            return false;
        }
        ...
        if (!newUser.password || newUser.password.length < 7 || newUser.password.length > 14) {
            swal.fire("Validation error", "Password must be between 7 and 14 characters.", "error");
            return false;
        }
        return true;
    };
